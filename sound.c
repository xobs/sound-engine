...
static void setInstrument(struct ltc_sound_engine *engine,
                          int voice_number,
                          const struct ltc_instrument *instrument,
                          ...
                          uint32_t release_time)
{
    engine->voices[voice_number].instrument = instrument;
    engine->voices[voice_number].attack_time = (attack_time * SAMPLE_RATE) / 1000;
    engine->voices[voice_number].attack_level = attack_level;
    engine->voices[voice_number].decay_time = (decay_time * SAMPLE_RATE) / 1000 + engine->voices[voice_number].attack_time;
    engine->voices[voice_number].sustain_level = sustain_level;
    engine->voices[voice_number].release_time = (release_time * SAMPLE_RATE) / 1000;
}

void setup(void)
{
    setInstrument(&engine,
                  0,
                  &sine_instrument,
                  450, 70,
                  250,
    ...
    setInstrument(&engine,
                  ...
                  40,
                  50);
    engine.voices[0].phase_accumulator = 995;
...
void loop(void)
{
    ...
    // Check to see if we need to step the sound state machine
    if (global_tick_counter >= (timesteps[current_note]))
    {
        //play_routine_step(&engine);
        if (velocities[current_note] == 0)
        {
            noteOff(&engine.voices[0]);
            current_note++;
        }
        else
        {
            noteOn(&engine.voices[0], midi_freq[notes[current_note]]);
            current_note++;
        }
    ...
    if ((global_tick_counter > VOICE_2_DELAY) && ((global_tick_counter - VOICE_2_DELAY) >= (timesteps[current_note_2])))
    {
        ...
        {
            noteOff(&engine.voices[1]);
            current_note_2++;
        }
        else
        {
            noteOn(&engine.voices[1], midi_freq[notes[current_note_2]]);
            current_note_2++;
        }
